...
function SaveInventory(source, offline)
    ...
    if success then
        TriggerEvent('tmg-log:server:CreateLog', 'saves', 'Inventory Synced', 'blue', 
            string.format("CID: %s | Items Saved: %d | Offline: %s", 
            PlayerData.citizenid, #saveTable, tostring(offline)))
    ...
    if isTable then
        ...
        if isArray then
            for i = 1, #items do
                ...
                checklist[name] = (checklist[name] or 0) + amount
                requirementsCount = requirementsCount + 1
            end
        else
            for name, reqAmount in pairs(items) do
                local lowerName = name:lower()
                checklist[lowerName] = (checklist[lowerName] or 0) + (tonumber(reqAmount) or 1)
                requirementsCount = requirementsCount + 1
            end
        end
    else
        checklist[items:lower()] = amount
        requirementsCount = 1
    end
...
function RemoveItem(identifier, item, amount, slot, reason)
    local itemName = item:lower()
    if not TMGCore.Shared.Items[itemName] then 
        return false 
    end
    ...
    if not targetSlot or not inventory[targetSlot] then return false end

    local inventoryItem = inventory[targetSlot]
    local amount = tonumber(amount) or 1

    if inventoryItem.amount < amount then return false end

    local isFullRemoval = (inventoryItem.amount == amount)
    local fieldPath = (collection == 'players') and "inventory." or "items."
    ...
    if isFullRemoval then
        ...
        inventory[targetSlot].amount = inventory[targetSlot].amount - amount
        updateOperation = { ["$set"] = { [fieldPath .. tostring(targetSlot) .. ".amount"] = inventory[targetSlot].amount } }
    end
    ...
    TriggerEvent('tmg-log:server:CreateLog', 'playerinventory', 'Item Removed', 'red', 
        string.format("**Target:** %s\n**Item:** %s\n**Amt:** %d\n**Slot:** %s\n**Reason:** %s", 
        identifier, itemName, amount, tostring(targetSlot), removeReason))
